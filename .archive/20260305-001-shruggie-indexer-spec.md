...
#### Directory metadata suppression
...
This feature is designed for workflows that consume only per-file sidecars (e.g., media libraries where directory-level summaries are unnecessary overhead).
...
On Windows, `ctime` (creation time) is also restored via `ctypes.windll.kernel32` using `CreateFileW` / `SetFileTime` / `CloseHandle`. The FILETIME value is computed from the Unix timestamp as `int((unix_seconds + 11644473600) * 10_000_000)`. This approach uses only the Python standard library (`ctypes` and `sys`) and requires no external dependency. If the `SetFileTime` call fails (e.g., permission denied), the failure is logged at WARNING level and the remaining timestamps (mtime/atime) are still applied. On non-Windows platforms, `ctime` restoration is not attempted because POSIX provides no standard API for setting file creation time.

> **Updated 2026-06-14:** Replaced the previous `pywin32` (`win32file.SetFileTime`) approach with a zero-dependency `ctypes.windll.kernel32` implementation. This eliminates the conditional `pywin32` import and the associated "pywin32 not available" fallback path that produced confusing DEBUG-level messages on systems without the optional dependency installed.
...
Controls whether `_directorymeta2.json` directory sidecar files are written during in-place output. The flag is a boolean pair: `--dir-meta` enables directory metadata (the default behavior), `--no-dir-meta` suppresses it. When `None` (not specified on the command line), the value is resolved from the configuration file or compiled default (`True`).

Suppression affects only directory-level sidecars — per-file `_meta2.json` sidecars are always written when `--inplace` is active. Auto-generated aggregate output files ending in `_directorymeta2.json` are also suppressed. Explicitly specified `--outfile` paths are never suppressed. Stdout output is never affected. See [§6.9](#69-json-serialization-and-output-routing), "Directory metadata suppression" for the full routing table.

Maps to `config.write_directory_meta` ([§7.1](#71-configuration-architecture)).
...
| 4a | `--inplace --no-dir-meta` | — | — | ✔ (files only) | New: in-place output without directory sidecars |
| 5a | `--outfile index.json --inplace --no-dir-meta` | — | ✔ | ✔ (files only) | New: aggregate + in-place without directory sidecars |
...
# Write per-file sidecars only (suppress directory metadata)
shruggie-indexer /path/to/media --inplace --no-dir-meta

# Combine with aggregate output — only explicit outfile is written
shruggie-indexer /path/to/media --outfile custom.json --inplace --no-dir-meta
```
...
The Output group contains three controls:
...
2. **Output path display** (read-only `CTkEntry`) — Shows the auto-computed output path based on the target and selected mode. For `View only`, displays *"(displayed in viewer)"*. For `Multi-file`, displays the target directory path with a supplemental note: *"Sidecar files written alongside originals."* When Multi-file is active and directory metadata is suppressed, the note changes to: *"(per-file sidecars only — directory summaries suppressed)"*.

3. **Write directory metadata checkbox** (`CTkCheckBox`) — Controls whether `_directorymeta2.json` sidecar files are emitted for directories during in-place output. Default: checked (enabled). Maps to `config.write_directory_meta` ([§7.1](#71-configuration-architecture)). An adjacent info-label (muted italic text) displays constraint explanations when the checkbox is disabled or when the setting triggers output mode adjustments. State persists across sessions.
...
| Target is a single file | Write directory metadata checkbox is disabled (not applicable to file targets). |
| Directory metadata unchecked + directory target | Single file mode unavailable (Single file produces only directory metadata, which would be empty). Falls back to Multi-file. Info-label: *"Single file mode produces only directory metadata. Switching to Multi-file."* |
| Meta Merge Delete + directory metadata unchecked | Multi-file is forced. The combination of destructive operation constraints and directory metadata suppression prevents Single file mode. |
...
> **Updated 2026-02-27:** The "Shared Settings (not yet available)" label is now rendered in non-italic red (`#cc0000`) text, making the unavailable status visually prominent as a warning rather than a subtle italic note. Read-only textbox heights are auto-sized to accommodate their default content without scrolling (lines × ~20px + padding), capped at 300px for textboxes with more than ~15 lines of content.
...
Each group renders its values in a read-only `CTkTextbox` widget with complete, untruncated default values — including long patterns such as the BCP 47 language-code alternation. Textbox heights are auto-sized to display all default content without scrolling (calculated as `min(lines × 20 + 10, 300)` pixels), capped at 300px for textboxes exceeding ~15 lines of content to maintain reasonable page length. Subsection descriptions are rendered as muted-color text below each subsection header. Each subsection is independently collapsible: clicking a subsection header toggles only that subsection without affecting siblings or the parent. The parent "Advanced Configuration" toggle controls overall visibility; when expanded, each subsection returns to its last-known individual state. All subsections default to collapsed on first launch. Individual subsection expanded/collapsed states persist across sessions via the session file.
...
**Region boundary clarity:** Wherever an anchored (non-scrollable) region borders a scrollable or background-matched content area, a uniform 1px separator line (`CTkFrame`, `height=1`, `fg_color="gray50"`) MUST be placed at the boundary edge to visually delineate the regions. This treatment is applied at the bottom edge of all page titles (Operations, Settings, and About) and the top edge of the Operations page bottom anchored region (progress/action area). All occurrences MUST use the identical visual style. Mixing different boundary treatments (e.g., shadows for titles but lines for bottom regions) is prohibited.

> **Updated 2026-02-27:** Extended the header-region bottom border convention to the About page title, completing consistent separator coverage across all three pages.
...
<a id="114a-parameter-logging-at-operation-start"></a>
#### Parameter logging at operation start

> **Added 2026-06-14:** Mandates INFO-level parameter summary at the beginning of every indexing and rollback operation.

Every operation entry point — CLI indexing, CLI rollback, GUI indexing, GUI rollback, and the core `plan_rollback()` function — MUST emit a single INFO-level log record summarizing all resolved operational parameters before processing begins. This record provides immediate diagnostic visibility into the settings governing the current invocation and is essential for debugging issues from log output alone.

**Core rollback (`plan_rollback()`).** The planner emits a summary containing: entry count, target directory, source directory, verify, force, flat, skip_duplicates, and restore_sidecars.

**CLI indexing (`index_cmd()`).** The command handler emits a summary after `load_config()` and implication propagation, containing: target, recursive, id_algorithm, compute_sha512, extract_exif, meta_merge, meta_merge_delete, rename, dry_run, output_stdout, output_file, output_inplace, and write_directory_meta.

**CLI rollback (`rollback_cmd()`).** The command handler emits a summary after `configure_logging()`, containing: meta2_path, target, source, recursive, flat, dry_run, verify, force, skip_duplicates, and restore_sidecars.

**GUI indexing and rollback.** The background thread handlers emit corresponding summaries before invoking the core engine, using the same parameter sets as the CLI commands.

The summary MUST appear before any item-level processing begins. It is emitted at `INFO` level so that it is visible with the `-v` CLI flag or the GUI's INFO log level setting. At WARNING level (the default), the summary is suppressed — this is correct because a user who has not opted into verbose output does not expect parameter dumps.
